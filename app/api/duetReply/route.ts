...
export async function POST(req: Request) {
  try {
    ...
    if (payload.role === "guest") {
      return jsonResponse({ ok: false, error: "Only the host can reply" }, 403)
    }
    ...
    if (duet.reply_count >= 1) {
      return jsonResponse({ ok: false, error: "Already replied (max 1 reply per duet)" }, 409)
    }
    ...
    if (!reply || typeof reply !== "string" || reply.trim().length === 0) {
      return jsonResponse({ ok: false, error: "reply field required" }, 400)
    }
