...
**Removing `as const` enum values narrows the type.** After killing 9 of 11 themes, `textAlign` became a union of `"center" | "center"` — effectively just `"center"`. TypeScript correctly flagged `theme.textAlign === "left"` as an impossible comparison. When deleting entries from a `as const` object, also remove properties that collapse to a single literal type and simplify the downstream code that branched on them.
